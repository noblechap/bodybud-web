...
import type { CheckInTemplate, CheckInTemplateItem } from "~/types/models";
...
const { checkInTemplates } = storeToRefs(coachingStore);
...
function goBack() {
  router.push(`/coaching/client/${clientId.value}`);
}

function loadTemplate(templateId: number) {
  const template = checkInTemplates.value.find((t: CheckInTemplate) => t.id === templateId);
  if (!template) {
  ...
  formData.value.items = (template.items || []).map((item: CheckInTemplateItem) => ({
    type: item.item_type,
    prompt: item.prompt || "",
...
async function assignCheckIn() {
  try {
    ...
    await coachingService.assignCheckIn(payload);
...
async function saveCheckInTemplate() {
  ...
  try {
    ...
    await coachingService.saveCheckInTemplate(payload);
  ...
  <div class="checkin-builder">
    ...
    <div class="builder-layout">
      ...
      <div class="config-panel">
        ...
        <v-card v-if="formData.items.length > 0" variant="flat">
          <v-card-title class="d-flex align-center">
            <v-icon color="primary" class="mr-2">
              mdi-chart-box
            </v-icon>
    ...
    <v-dialog v-model="showPreview" max-width="600">
      <v-card>
        ...
        <v-card-text class="preview-content">
          ...
          <div v-if="formData.items.length > 0" class="preview-items">
            <div
            ...
            >
              <div class="preview-item-header">
                ...
                <div class="preview-item-number">
                  {{ index + 1 }}
                </div>
              </div>
              <div class="preview-item-prompt">
                {{ item.prompt }}
              </div>
            </div>
          </div>
...
.required-chip :deep(.v-chip__underlay) {
...
.checkin-item :deep(.v-text-field) {
  margin-bottom: 12px;
}
.checkin-item :deep(.v-input--density-comfortable) {
