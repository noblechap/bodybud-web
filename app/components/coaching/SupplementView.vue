...
import { useSupplementService } from "~/composables/services/useSupplementService";
import { useClientStore } from "~/store/clientStore";

const clientStore = useClientStore();
const supplementService = useSupplementService();
...
const currentSupplement = ref<Partial<Supplement>>({
  id: undefined,
  name: "",
  dose: "",
  notes: "",
...
function openEditDialog(supplement: Supplement) {
...
async function saveSupplement() {
  ...
  try {
    if (isEditing.value && currentSupplement.value.id) {
      await supplementService.updateSupplement(currentSupplement.value as Supplement);
    }
    else {
      await supplementService.addSupplement({
        name: currentSupplement.value.name!,
        dose: currentSupplement.value.dose!,
        notes: currentSupplement.value.notes,
      });
    }
    showDialog.value = false;
  }
...
async function deleteSupplement() {
  if (!currentSupplement.value.id)
    return;

  try {
    await supplementService.deleteSupplement(currentSupplement.value.id);
  ...
  <v-container fluid class="pa-4 pa-md-6">
    ...
    >
      ...
      <p class="text-medium-emphasis mb-4">
        Add a supplement to get started.
      </p>
      <v-btn
        color="primary"
        variant="flat"
